// Package catalog embeds the installable application catalog that ships with the
// server.
//
...
// follow symlinks. Keeping the catalog at applications/ — where someone adding an
...
// FS is the catalog compiled into the server binary. Every application is
// applications/<id>, which is also the layout an uploaded package carries, so the
