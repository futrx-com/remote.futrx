...
- **The optional IDE is still reachable across the shared LXD bridge.** The public IDE routes check project membership and a running Code Server install, but project containers have no lateral network isolation. A process inside another project container can connect directly to the editor's unauthenticated port while it is installed. See the [threat model](threat-model.md).
...
- **Launch provisioning is best-effort.** Credential seeding, skill links, and browser tooling swallow their errors, so a container can come up missing credentials or browser tooling. Code Server is now installed explicitly as a project application; install errors surface through its application status.
