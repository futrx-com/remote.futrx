...
| Kimi | Reads the private native server model catalog and configured default, including managed/environment aliases. Exposes supported Thinking controls, Plan mode and native approval choices independently of sandbox controls. | Catalog/startup failure returns an Auto-model fallback. Missing defaults preserve the catalog with a warning. |
