...
Prefix every class with something unique to your application (`.mysql-ext-`, `.pg-`,
...
`icon` in `application.json` takes either form:

```json
"icon": "database"              // a built-in key
"icon": "ui/assets/logo.svg"    // a file this application ships
```
...
A value starting with `ui/` is a path into your application's own `ui/` directory,
served through the same authenticated asset route as the rest of the extension.
Any format the browser renders works.

An application may ship a `ui/` that contains **nothing but an icon** — it does not
