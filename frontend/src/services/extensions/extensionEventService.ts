...
// something finished, so an application's `ui/` can react without polling and
// without patching anything global.
//
// Subscriptions are keyed by application id rather than held anonymously. That is
// what lets the extension host drop an uninstalled application's handlers: the
...
class ExtensionEventService {
   ...
   * handler twice registers it twice, which is what an application reloading its own
  ...
  emit<Name extends ExtensionEventName>(
    ...
    for (const subscription of [...this.subscriptions]) {
      ...
      try {
        ...
        console.error(
          `[extensions] ${subscription.applicationId} failed handling ${name}`,
          error,
        );
      }
    }
  ...
  /** Drops every subscription an application registered. Called when it is removed. */
