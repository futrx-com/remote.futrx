...
interface Subscription {
  ...
  // The handler is stored erased: subscribe() has already checked the pairing
  // of name and handler at its call site, and emit() re-narrows on the way out.
 ...
 */
export function subscribeExtensionEvent<Name extends ExtensionEventName>(
  imageId: string,
  name: Name,
 ...
 */
export function emitExtensionEvent<Name extends ExtensionEventName>(
  name: Name,
...
export function removeExtensionEventSubscriptions(imageId: string): void {
  for (const subscription of [...subscriptions]) {
    if (subscription.imageId === imageId) subscriptions.delete(subscription);
  }
}

/** Test seam: forgets every subscription. */
export function resetExtensionEvents(): void {
  subscriptions.clear();
}
